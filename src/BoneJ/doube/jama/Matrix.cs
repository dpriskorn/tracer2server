...
namespace gov.nist.math.jama
{
    class Matrix
    {
        ...
        {
            double[][] C = Matrix.CreateArray(m, n);
            for (int i = 0; i < m; i++)
            {
                for (int j = 0; j < n; j++)
                {
        ...
        public double dotProduct(Matrix B)
        {
            ...
            if (m != 1 && n != 1)
            {
                throw new ArgumentException("Matrix is not a row or column vector");
            }
        ...
        public Matrix crossProduct(Matrix B)
        {
            ...
            {
                throw new ArgumentException("Matrix must be a 3-element row or column vector");
            }
            return X;
        }
        ...
        {
            double[][] ones = Matrix.CreateArray(m, n);
            for (int i = 0; i < m; i++)
            {
                for (int j = 0; j < n; j++)
                {
        ...
        {
            double[][] eye = Matrix.CreateArray(m, n);
        ...
        {
            int min = Math.Min(m, n);
            double[][] diag = Matrix.CreateArray(min, 1);
            for (int i = 0; i < min; i++)
            {
                diag[i][0] = get(i, i);
            }
        ...
        public static double[][] CreateArray(int nHeight, int nWidth)
        {
            double[][] adResult = new double[nHeight][];
            for (int x = 0; x < adResult.Length; x++)
            {
                adResult[x] = new double[nWidth];
            }
            return adResult;
        }
    }
